...
function indexOf(array: Array<any>, obj: any, start: number) {
...
function Instruction(type:any, value:any) {
  this.type = type;
...
function simplify(tokens, unaryOps, binaryOps, ternaryOps, values) {
  var nstack:Array<any> = [];
  var newexpression:Array<any> = [];
  var n1, n2, n3;
  var f;
...
function getSymbols(tokens:Array<any>, symbols:Array<any>) {
  for (var i = 0, L = tokens.length; i < L; i++) {
    var item = tokens[i];
...
Parser.parse = function (expr:string) {
  return new Parser().parse(expr);
};

Parser.evaluate = function (expr:string, variables:any) {
